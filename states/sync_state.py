...
class SyncStateHandler(StateHandler):
    def __init__(self, instance: ThisInstance):
        ...
        self._recieved_buffer_begin: Dict[
            int, bool
        ] = {}  # instance_index -> received begin flag

        self._message_handler_callback: Callable[
            [int, IncomingMessage], StateResult | None
        ] = None
        self._buffer_handler_callback: Callable[
            [int, IncomingBuffer], StateResult | None
        ] = None
    ...
    async def handle_state(self, current_state: int) -> StateResult | None:
        if self._state_handler_callback and callable(
            self._state_handler_callback
        ):
    ...
    async def handle_message(
        ...
        if incoming_message.msg_type == ClusterMessageType.DISTRIBUTE_PROMPT:
            ...
            try:
                ...
                logger.info(
                    "Successfully posted prompt to local ComfyUI instance"
                )

            except requests.exceptions.RequestException as e:
                logger.error(f"Error posting prompt: {str(e)}")

        if self._message_handler_callback is None or not callable(
            self._message_handler_callback
        ):
            return None
        return await self._message_handler_callback(
            current_state, incoming_message
        )

    async def handle_buffer(
        self, current_state: int, incoming_buffer: IncomingBuffer
    ) -> StateResult | None:
        if self._buffer_handler_callback is None or not callable(
            self._buffer_handler_callback
        ):
            return None
        return await self._buffer_handler_callback(
            current_state, incoming_buffer
        )
    ...
    def _register_delegates(
        self, handle_message_callback, handle_buffer_callback, tick_callback
    ):
    ...
    async def _begin_buffer_sender(
        ...
        to_instance_ids: list[int] | None = None,
    ):
        all_instanced_received_buffer: asyncio.Future = (
            asyncio.get_running_loop().create_future()
        )
    ...
    def _tensor_to_compressed_bytes(self, tensor: torch.tensor) -> bytes:
        ...
        try:
            ...
            logger.info(
                f"Compressed tensor payload size: {len(packed_data)} bytes"
            )
            return packed_data

        except Exception as e:
            logger.error(
                f"PNG compression failed: {str(e)}\n{traceback.format_exc()}"
            )
            return tensor.numpy().tobytes()
    ...
    def _compressed_bytes_to_tensor(self, buffer: bytes) -> torch.tensor:
        ...
        try:
            ...
            try:
                logger.info(
                    f"Decompressing tensor payload size: {len(buffer)} bytes"
                )
    ...
    def split_batch(
        ...
        start_idx = instance_id * base_per_instance + min(
    ...
    async def _begin_fanout_emitter(self, tensor: torch.Tensor):
        ...
        for instance_id in range(EnvVars.get_instance_count()):
            ...
            await self._send_buffer_descriptor(
                image_tensor, FANOUT_EXPECTED_MSG_KEY
            )
    ...
    async def _begin_gather_tensors(
        ...
        on_instance_received_buffer: List[asyncio.Future] = [
            None
        ...
        received_buffer_shapes: list[tuple[int]] = [
            None
        ] * EnvVars.get_instance_count()
        ...
        for current_emitter_instance_id in range(EnvVars.get_instance_count()):
            if current_emitter_instance_id == EnvVars.get_instance_index():
                ...
                await self._send_buffer_descriptor(
                    tensor, GATHER_EXPECTED_MSG_KEY
                )
                await emitter.begin()
                await all_instanced_received_buffer
                ...
                buffer_descriptor: ClusterDistributeBufferDescriptor = (
                    await self._receive_buffer_descriptor(
                        GATHER_EXPECTED_MSG_KEY
                    )
                )
                await receivers[current_emitter_instance_id].begin()

                result = await on_instance_received_buffer[
                    current_emitter_instance_id
                ]
                buffer = result.get_buffer()
                if len(buffer) == 0:
                    raise Exception(
                        f"Failed to receive buffer from instance {current_emitter_instance_id}"
                    )
                received_buffers[current_emitter_instance_id] = buffer
                received_buffer_shapes[current_emitter_instance_id] = tuple(
                    buffer_descriptor.buffer_shape
                )
    ...
    async def _receive_buffer_descriptor(
        ...
        if (
            not buffer_descriptor.buffer_shape
            or len(buffer_descriptor.buffer_shape) == 0
        ):
            raise Exception(
                "Invalid buffer descriptor - missing shape information"
            )

        logger.debug(
            f"Received buffer descriptor from instance {incoming_message.sender_instance_id} for tensor with shape: {buffer_descriptor.buffer_shape}"
        )
    ...
    async def begin_receiver(self, expected_msg_key: int) -> torch.Tensor:
        buffer_descriptor: ClusterDistributeBufferDescriptor = (
            await self._receive_buffer_descriptor(expected_msg_key)
        )
        ...
        if tensor.shape != expected_shape:
            ...
            try:
                ...
                logger.warning(
                    "Creating new tensor with correct shape. Data may be corrupted."
                )
                tensor = torch.zeros(
    ...
    async def begin_sender(
        self, tensor: torch.Tensor, to_instance_ids: list[int] | None = None
    ) -> torch.Tensor:
        return await self._begin_buffer_sender(
            tensor, FANIN_EXPECTED_MSG_KEY, to_instance_ids
        )

    async def begin_gathering_tensors(
        self, tensor: torch.Tensor
    ) -> torch.Tensor:
        ...
        for buffer, shape in zip(buffers, shapes):
            try:
                ...
                if decompressed_tensor.shape != shape:
                    ...
                    try:
                        decompressed_tensor = decompressed_tensor.reshape(
                            shape
                        )
                    except RuntimeError as e:
                        logger.error(f"Failed to reshape tensor: {str(e)}")
