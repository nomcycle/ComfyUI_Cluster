...
class BufferBeginEvent:
    def __init__(
        self, sender_instance_id: int, buffer_type: int, chunk_count: int
    ):
...
class Receiver(SyncHandler):
    ...
    async def begin(self):
        ...
        result = (
            await self._udp_message_handler.await_expected_message_thread_safe(
        ...
        with self._thread_lock:
            ...
            logger.debug(
                f"Received buffer begin message from instance {distribute_buffer.instance_index}, expecting {distribute_buffer.chunk_count} chunks"
            )
            self._sender_instance_id = distribute_buffer.instance_index
            self._expected_buffer_type = distribute_buffer.buffer_type
            self._expected_chunk_ids = list(
                range(distribute_buffer.chunk_count)
            )
            self._dependency_chunks = {}
            self._chunks_bitfield = np.zeros(
                distribute_buffer.chunk_count, dtype=np.bool_
            )
    ...
    def _buffer_progress(self):
        if len(self._chunks_bitfield) == 0:
            raise ValueError(
                "No chunks field available - cannot calculate buffer progress"
            )
        total_chunks = np.sum(self._chunks_bitfield)  # Count True flags
        expected_total = (
            len(self._expected_chunk_ids) if self._expected_chunk_ids else 0
        )
    ...
    async def tick(self):
        ...
        with self._thread_lock:
            ...
            incoming_queue_size = (
                self._udp_buffer_handler.get_incoming_buffer_queue_size()
            )
            time_since_last_packet = (
                self._udp_buffer_handler.get_time_since_last_packet()
            )
            time_since_last_poll = (
                current_time - self._time_since_polling_chunk_progress
            )

            if (
                incoming_queue_size == 0
                or time_since_last_packet < 0.025
                or time_since_last_poll < 0.025
            ):
                return await asyncio.sleep(0.001)
            ...
            logger.debug(
                f"Instance {EnvVars.get_instance_index()}: Have {total_chunks}/{expected_total} chunks"
            )
            if expected_total > 0:
                if total_chunks < expected_total:
                    ...
                    end_index = min(
                        first_missing_index
                        + BUFFER_RESEND_MAX_REQUEST_SIZE * 8,
                        len(self._chunks_bitfield),
                    )

                    # Extract the window of missing chunks
                    window_bitfield = ~self._chunks_bitfield[
                        first_missing_index:end_index
                    ]
                    ...
                    logger.debug(
                        f"Requesting resend for window size: {window_size} starting at index {first_missing_index}, byte length: {len(window_bytes)}"
                    )
                    ...
                    await self._udp_message_handler.send_and_wait(
                        message, self._sender_instance_id
                    )
                    ...
                    logger.info(
                        f"All chunks received from instance {self._sender_instance_id}, joining buffers"
                    )
    ...
    async def handle_buffer(
        ...
        with self._thread_lock:
            ...
            if (
                sender_instance_id == EnvVars.get_instance_index()
                or sender_instance_id != self._sender_instance_id
            ):
                raise Exception(
                    f"Received buffer from unexpected instance {sender_instance_id}, expected {self._sender_instance_id}"
                )
            ...
            if sender_instance_id is not None:
                ...
                self._dependency_chunks[chunk_id] = buffer_view[
                    SyncHandler.HEADER_SIZE :
                ].tobytes()
                self._chunks_bitfield[chunk_id] = True

                # Check progress less frequently
                if chunk_id % 5000 == 0 or (
                    chunk_id == len(self._expected_chunk_ids) - 1
                ):
                    total_chunks, expected_total = self._buffer_progress()
                    logger.info(
                        f"Received chunk {chunk_id} from instance {sender_instance_id}. Total: {total_chunks}/{expected_total}"
                    )
