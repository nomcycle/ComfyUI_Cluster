...
from .utils import prepare_loop, get_subgraph, replace_nodes_in_subgraph, add_preview_nodes_after_type
...
class ClusterFanOutBase(ClusterTensorNodeBase, ClusterNodePair):
    ...
    async def _sync_operation(self, instance, unique_id, input):
        if EnvVars.get_instance_index() == 0:
            ...
            subgraph = get_subgraph(unique_id, self.get_start_type(), self.get_pair_end_type())

            # Apply modifications as needed
            subgraph = replace_nodes_in_subgraph(subgraph, self.get_start_type(), ClusterListenTensorBroadcast.__name__, clear_inputs=True)
            subgraph = add_preview_nodes_after_type(subgraph, self.get_pair_end_type())
