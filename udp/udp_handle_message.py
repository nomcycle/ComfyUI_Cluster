...
class UDPMessageHandler(UDPBase):
    ...
    def _handle_incoming_packet(self, incoming_packet: IncomingPacket):
        try:
            if incoming_packet.get_is_buffer():
                return
            incoming_msg: IncomingMessage = self._validate_incoming_message(
                incoming_packet
            )
            if not incoming_msg:
                return
                ...
                for instance_id, pending_ack in list(
                    ...
                    if pending.should_retry(instance_id, current_time):
                        if pending.has_exceeded_retries(instance_id):
                            ...
                            self._queue_outgoing_to_instance(
    ...
    def _validate_incoming_message(self, packet: IncomingPacket):
        ...
        if (
            ...
            and packet.sender_addr in self.get_cached_local_addreses()
        ):
            return None
    ...
    def _send_ack(
        self, message_id: int, addr_and_port: tuple[str, int] | None = None
    ):
    ...
    def _create_pending_message(
        self, message_id: int, message, instance_id: int | None = None
    ):
    ...
    async def send_and_wait_thread_safe(
        self, message, instance_id: int | None = None
    ):
        return await self._execute_coroutine_thread_safe(
    ...
    def _queue_outgoing_to_addr(
        self, packet, addr_and_port: tuple[str, int] | None = None
    ):
